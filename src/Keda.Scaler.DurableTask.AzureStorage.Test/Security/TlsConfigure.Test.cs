...
public sealed class TlsConfigureTest : IDisposable
{
    ...
    {
        _ = Assert.Throws<ArgumentNullException>(() => new TlsConfigure(null!, Substitute.For<IOptions<TlsServerOptions>>(), NullLoggerFactory.Instance));
        _ = Assert.Throws<ArgumentNullException>(() => new TlsConfigure(Options.Create<TlsClientOptions>(null!), Substitute.For<IOptions<TlsServerOptions>>(), NullLoggerFactory.Instance));
    }

    [Fact]
    public void GivenNullTlsServerOptions_WhenCreatingTlsConfigure_ThenThrowArgumentNullException()
    {
        _ = Assert.Throws<ArgumentNullException>(() => new TlsConfigure(Substitute.For<IOptions<TlsClientOptions>>(), null!, NullLoggerFactory.Instance));
        _ = Assert.Throws<ArgumentNullException>(() => new TlsConfigure(Substitute.For<IOptions<TlsClientOptions>>(), Options.Create<TlsServerOptions>(null!), NullLoggerFactory.Instance));
    }
    ...
    {
        using TlsConfigure configure = new(Options.Create(new TlsClientOptions()), Options.Create(new TlsServerOptions()), NullLoggerFactory.Instance);
